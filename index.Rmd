...
poke_join %>% filter(Total > mean(Total, na.rm = T))
...
poke_join %>% select(Name, is_default, Legendary)
poke_join %>% mutate(BMI = (weight/(height*height))*10)

# your wrangling code
poke_join %>% group_by(Generation) %>% pivot_wider() %>% summarize(count=n()) %>% arrange(-count)
poke_join %>% filter(str_detect(Name, ".*-.*"))
```
